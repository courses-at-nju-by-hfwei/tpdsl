...
/**
 * A scope is a dictionary of symbols that are grouped together
 * by some lexical construct in the input language.
 * Examples include structs, functions, {...} code blocks,
 * argument lists, etc...
 *
 ...
 *
 * This is distinguished from the parent scope.
 * The parent scope is usually the enclosing scope,
 * but in the case of inheritance, it is the superclass rather than the enclosing scope.
 * Otherwise, the global scope would be considered the parent scope of a class.
 * When resolving symbols, we look up the parent scope chain
 * not the enclosing scope chain.
 *
 * For convenience of code using this library,
 * I have added a bunch of methods one can use
 * to get lots of useful information from a scope,
 * but they don't necessarily define what a scope is.
 */
public interface Scope {
	/**
	 * Often scopes have names like function or class names.
	 * For unnamed scopes like code blocks,
	 * you can just return "local" or something.
	 */
	String getName();
	 ...
	 * E.g., if this scope is a function, the enclosing scope could be a class.
	 * The BaseScope class automatically adds this to nested scope list of s.
	 */
	 ...
	 * The order of insertion into the scope is the order returned in this list.
	 */
